...
public class stem {
    public static void main(String[] args){
        ...
        l.stream().sorted((s1,s2)->s1.name.compareTo(s2.name)).forEach(s->s.show());

        System.out.println("Top 3 Students By Marks");
        l.stream().sorted((s1,s2)->s2.marks-s1.marks).limit(3).forEach(s->s.show());

        double avg = l.stream().mapToInt(s->s.marks).average().getAsDouble();
        System.out.println("Average Marks: "+avg);

        Map<String,List<stud>> gradeMap =l.stream().collect(Collectors.groupingBy(s->s.getGrade()));
